...
sudo docker exec "$(cat /tmp/container_id)" sudo mkdir /var/travis_test_work
sudo docker exec "$(cat /tmp/container_id)" sudo rpm -Uvh http://dl.fedoraproject.org/pub/epel/7/x86_64/e/epel-release-7-2.noarch.rpm
sudo docker exec "$(cat /tmp/container_id)" sudo yum -y install python-virtualenv
