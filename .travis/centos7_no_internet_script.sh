...
sudo docker exec "$(cat /tmp/container_id)" yum -y install python-virtualenv libffi-devel openssl-devel sudo
...
sudo docker exec "$(cat /tmp/container_id)" mkdir /var/travis_test_source/.travis/roles
sudo docker exec "$(cat /tmp/container_id)" ln -s /var/travis_test_source/ /var/travis_test_source/.travis/roles/karlmdavis.rcm
